"""Tests for sync_gitlab_commits with SyncTask status updates.
...
class TestSyncGitlabCommitsStatus:
    """Test cases for sync_gitlab_commits with status updates."""
    ...
    def mock_gitlab_data_source(self):
        ...
        with patch('app.tasks.sync_tasks.GitLabDataSource') as mock_class:
            ...
            mock_instance.sync_commits = AsyncMock(return_value={
                "total": 10,
                "processed": 10,
                "failed": 0,
                "errors": []
    ...
    def test_sync_gitlab_commits_accepts_task_id(self):
        """Test that sync_gitlab_commits accepts task_id parameter."""
        from app.tasks.sync_tasks import sync_gitlab_commits

        # Assert task function signature includes task_id
        import inspect
        sig = inspect.signature(sync_gitlab_commits.run)
    ...
    def test_sync_gitlab_commits_calls_update_status_running(
        self, mock_update_sync_task_status, mock_gitlab_data_source
    ):
        """Test that sync_gitlab_commits calls update_sync_task_status with 'running' at start."""
        from app.tasks.sync_tasks import sync_gitlab_commits

        task_id = 123

        # Call the task
        result = sync_gitlab_commits.run(project_id=1, task_id=task_id)
    ...
    def test_sync_gitlab_commits_calls_update_status_completed(
        self, mock_update_sync_task_status, mock_gitlab_data_source
    ):
        """Test that sync_gitlab_commits calls update_sync_task_status with 'completed' on success."""
        from app.tasks.sync_tasks import sync_gitlab_commits
        ...
        mock_update_sync_task_status.assert_any_call(
            task_id, "completed", records_processed=10, records_failed=0
        )

    def test_sync_gitlab_commits_calls_update_status_failed(
        self, mock_update_sync_task_status
    ):
        """Test that sync_gitlab_commits calls update_sync_task_status with 'failed' on error."""
        from app.tasks.sync_tasks import sync_gitlab_commits
    ...
    def test_sync_gitlab_commits_without_task_id(self, mock_gitlab_data_source):
        """Test that sync_gitlab_commits works without task_id (backward compatibility)."""
        from app.tasks.sync_tasks import sync_gitlab_commits

        # Call the task without task_id
        result = sync_gitlab_commits.run(project_id=1)
