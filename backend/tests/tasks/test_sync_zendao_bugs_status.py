"""Tests for sync_zendao_bugs with SyncTask status updates.
...
class TestSyncZendaoBugsStatus:
    """Test cases for sync_zendao_bugs with status updates."""
    ...
    def mock_zendao_data_source(self):
        ...
        with patch('app.tasks.sync_tasks.ZenTaoDataSource') as mock_class:
            ...
            mock_instance.sync_bugs = AsyncMock(return_value={
                "total": 15,
                "processed": 15,
                "failed": 0,
                "errors": []
    ...
    def test_sync_zendao_bugs_accepts_task_id(self):
        """Test that sync_zendao_bugs accepts task_id parameter."""
        from app.tasks.sync_tasks import sync_zendao_bugs

        # Assert task function signature includes task_id
        import inspect
        sig = inspect.signature(sync_zendao_bugs.run)
    ...
    def test_sync_zendao_bugs_calls_update_status_running(
        self, mock_update_sync_task_status, mock_zendao_data_source
    ):
        """Test that sync_zendao_bugs calls update_sync_task_status with 'running' at start."""
        from app.tasks.sync_tasks import sync_zendao_bugs

        task_id = 123

        # Call the task
        result = sync_zendao_bugs.run(project_id=1, task_id=task_id)
    ...
    def test_sync_zendao_bugs_calls_update_status_completed(
        self, mock_update_sync_task_status, mock_zendao_data_source
    ):
        """Test that sync_zendao_bugs calls update_sync_task_status with 'completed' on success."""
        from app.tasks.sync_tasks import sync_zendao_bugs
        ...
        mock_update_sync_task_status.assert_any_call(
            task_id, "completed", records_processed=15, records_failed=0
        )

    def test_sync_zendao_bugs_calls_update_status_failed(
        self, mock_update_sync_task_status
    ):
        """Test that sync_zendao_bugs calls update_sync_task_status with 'failed' on error."""
        from app.tasks.sync_tasks import sync_zendao_bugs
        ...
        with patch.object(ZenTaoDataSource, 'sync_bugs', new_callable=AsyncMock) as mock_sync:
            ...
            with patch.object(sync_zendao_bugs, 'retry') as mock_retry:
    ...
    def test_sync_zendao_bugs_without_task_id(self, mock_zendao_data_source):
        """Test that sync_zendao_bugs works without task_id (backward compatibility)."""
        from app.tasks.sync_tasks import sync_zendao_bugs

        # Call the task without task_id
        result = sync_zendao_bugs.run(project_id=1)
