...
describe('TrendChart', () => {
  ...
  it('should support xAxis config object', () => {
    const wrapper = mount(TrendChart, {
      props: {
        xAxis: {
          name: '时间',
    ...
    expect(wrapper.vm.chartOption.xAxis?.name).toBe('时间')
    expect(wrapper.vm.chartOption.xAxis?.axisLabel?.rotate).toBe(45)
  })

  it('should support yAxis config object', () => {
    ...
    expect(wrapper.vm.chartOption.yAxis?.max).toBe(1000)
